...
class MasterGAN:
    ...
    def build_generative_model(self):
        ...
        noise_output = layers.Conv2DTranspose(16, (5, 5), strides=(2, 2), padding='same', use_bias=False)(noise_output)
        noise_output = layers.BatchNormalization()(noise_output)
        noise_output = layers.LeakyReLU()(noise_output)
        noise_output = layers.Conv2DTranspose(16, (5, 5), strides=(2, 2), padding='same', use_bias=False)(noise_output)
        noise_output = layers.BatchNormalization()(noise_output)
        ...
        noise_output = layers.Conv2DTranspose(4, (5, 5), strides=(2, 2), padding='same', use_bias=False)(noise_output)
        ...
        final_output = Concatenate(axis=1)([malware_input, noise_output*255])
        #final_output = Concatenate(axis=1)([malware_input, (noise_output*127.5+127.5)])
        #final_output = Concatenate(axis=2)([malware_input, noise_output*255])
        generator = Model(inputs=[malware_input,noise_input], outputs=[final_output])
        #print(generator.summary())
        # tf.keras.utils.plot_model(
        #     generator,to_file='/home/qian/Masterproject/PROJECT/Diagrams/generator_diagram.png', show_shapes=True, #show_dtype=True,
